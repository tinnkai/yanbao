...
type baseCheckOutInterface interface {
	...
	CalculationOrderAmount(orderGoods []models.OrderGoodsInfo)
}
...
type BaseOrderInfo struct {
	GoodsList   []models.OrderGoodsInfo `json:"goodsList"`
	TicketList  []models.OrderTicket    `json:"ticketList"`
	FreightInfo models.OrderFreight     `json:"freightInfo"`
...
// 获取商品信息列表
func (this *BasePost) GetGoodsList() ([]models.Goods, error) {
	...
	goodsList, err := models.GetGoodsList(ids)
	if err != nil {
...
// 获取订单商品信息列表
func (this *BasePost) GetOrderGoodsList(goodsList []models.Goods) []models.OrderGoodsInfo {

	// 订单商品列表
	var orderGoodsList []models.OrderGoodsInfo
	...
	for _, gv := range goodsList {
		...
		orderGoods := models.OrderGoodsInfo{
			Id:             gv.Id,
			Name:           gv.Name,
			Image:          "",
			Type:           gv.GoodsType,
...
// 查询订单赠品
func (this *BasePost) GetOrderGoodsGiftList(orderGoodsList []models.OrderGoodsInfo) error {
	// 商品id
	...
	// 获取订单商品赠品
	giftList, err := models.GetOrderGiftList(goodsIds)
	if err != nil {
		return err
	}

	for ogi, ogv := range orderGoodsList {
		giftInfo := make([]models.OrderGift, 0)
...
// 计算订单金额
func (this *BaseOrderInfo) CalculationOrderAmount(orderGoods []models.OrderGoodsInfo) {
